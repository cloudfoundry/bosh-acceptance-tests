...
describe 'check vcap password correct', type: 'os' do
  ...
  it 'vcap should exist in shadow and user_data.json' do
    ...
    output_shadow = bosh_ssh('batlight', 0, "cat /etc/shadow | grep #{passwd}").output
    running_services_shadow = output_shadow.split("\n").uniq
    expect(running_services_shadow).to include(passwd)

    output_json = bosh_ssh('batlight', 0, "cat /var/vcap/bosh/user_data.json | grep #{passwd}").output
